...
Site Builder 本体は **PolyForm Noncommercial License 1.0.0（非商用限定・商用利用禁止）**
で提供されます（`LICENSE` 参照）。本体のライセンスとは別に、以下の同梱・参照ソフトウェアには
**それぞれのライセンス（MIT / SIL OFL）が引き続き適用**され、これらは商用利用も可能です。
本体を再配布する際は、以下の各ライセンス表記を保持してください。
